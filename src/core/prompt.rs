...
/// Heuristic mode suggestion. Returns the suggested mode id.
///
/// Previously returned `(mode_id, reason)` with a human-readable rationale
/// string, but `Suggestion.reason` was removed (R8-4) and every call site
/// discards the second tuple element. Seven allocations per call for dead
/// data — collapsed to just the id.
pub fn suggest_mode(text: &str, context: &AppContext) -> String {
...
mod tests {
    ...
    fn suggest_long_text_returns_shorter() {
        ...
        let m = suggest_mode(&text, &ctx("neutral", "general"));
        assert_eq!(m, "shorter");
    }

    #[test]
    fn suggest_short_text_returns_expand() {
        let m = suggest_mode("just a few words here", &ctx("neutral", "general"));
        assert_eq!(m, "expand");
    }
    ...
    fn suggest_email_formal_context_returns_formal() {
        ...
        let m = suggest_mode(text, &ctx("professional", "email"));
        assert_eq!(m, "formal");
    }
    ...
    fn suggest_technical_context_returns_rewrite() {
        ...
        let m = suggest_mode(text, &ctx("technical", "code editor"));
        assert_eq!(m, "rewrite");
    }
