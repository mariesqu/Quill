...
///
/// Only the TCP/TLS connect step is bounded — a 10s ceiling so an unreachable
/// host fails fast instead of parking the streaming future indefinitely.
/// We do NOT set a total request timeout: legitimate streaming responses take
/// minutes and the engine has its own `cancel_tx` to abort user-cancelled
/// streams (see `streaming::run_single_stream`).
...
pub fn build_provider(cfg: &Config) -> Arc<dyn Provider> {
    match cfg.provider.as_str() {
        ...
        // Shell-out to the local Claude Code CLI — uses the user's logged-in
        // Anthropic session, no API key here. See `claude_cli.rs`.
        "claude-cli" | "claude_cli" | "claude" => Arc::new(claude_cli::ClaudeCliProvider::new(cfg)),
...
/// Per-chunk read timeout for SSE streams. Catches providers that open a
/// connection then stall without sending any data. 120 s is generous —
/// legitimate models may pause mid-generation for reasoning or rate-limit
/// retries, but never silently for two minutes.
pub(crate) const SSE_CHUNK_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(120);
...
/// is always safe.
///
/// Each chunk-read is bounded by `SSE_CHUNK_TIMEOUT`; a timeout ends the
/// stream naturally (upstream `streaming::run_single_stream` already
/// treats `None` as natural completion).
    ...
    let text_stream = stream::unfold(
        ...
        |(mut stream, mut buf)| async move {
            loop {
                ...
                // No complete line yet — pull more bytes, bounded.
                match tokio::time::timeout(SSE_CHUNK_TIMEOUT, stream.next()).await {
                    Ok(Some(Ok(bytes))) => buf.extend_from_slice(&bytes),
                    Ok(_) => return None,
                    Err(_) => {
                        tracing::warn!(
                            "SSE stream chunk read exceeded {:?} — ending stream",
                            SSE_CHUNK_TIMEOUT
                        );
                        return None;
                    }
...
///
/// Extracts the duplication from the three near-identical providers
/// (openai / openrouter / generic). Caller supplies the URL, JSON body,
/// optional bearer token (some endpoints allow anonymous access — e.g.
/// local LM Studio), and any additional headers (e.g. OpenRouter's
/// HTTP-Referer + X-Title attribution).
///
/// Returns a streaming `ChunkStream` on 2xx, or a friendly error string
/// via `friendly_error` on any other status or network failure.
pub async fn post_openai_chat(
    url: &str,
