...
use super::{ChunkStream, Provider, HTTP_CLIENT, SSE_CHUNK_TIMEOUT};
        ...
        let text_stream = stream::unfold(
            ...
            |(mut stream, mut buf)| async move {
                loop {
                    ...
                    // Bound each chunk read. An Ollama server that opened
                    // the connection then fell silent would otherwise park
                    // this future indefinitely.
                    match tokio::time::timeout(SSE_CHUNK_TIMEOUT, stream.next()).await {
                        Ok(Some(Ok(bytes))) => buf.extend_from_slice(&bytes),
                        Ok(_) => return None,
                        Err(_) => {
                            tracing::warn!(
                                "Ollama stream chunk read exceeded {:?} — ending stream",
                                SSE_CHUNK_TIMEOUT
                            );
                            return None;
                        }
