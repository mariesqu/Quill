...
/// * `enabled = false`  → monitor stays alive but skips emission.
/// * receiver dropped  → monitor thread exits cleanly.
/// * mute window active → monitor updates `last_text` but skips emission.
pub fn start_clipboard_monitor(enabled: Arc<AtomicBool>) -> mpsc::Receiver<String> {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            // ALWAYS refresh `last_text` from the live clipboard, even when
            // the monitor is disabled. Otherwise, enabling the monitor mid-
            // session would compare against an hours-old value and fire on
            // whatever text happened to be in the clipboard at that moment.
            ...
            // Disabled → tracking only, no emission.
