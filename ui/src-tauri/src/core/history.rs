...
pub fn get_recent(limit: usize, language: Option<&str>) -> Result<Vec<HistoryEntry>> {
    let conn = open()?;
    let lang_filter = language.filter(|l| *l != "auto");

    let rows: Vec<HistoryEntry> = match lang_filter {
        Some(lang) => {
            let mut stmt = conn.prepare(
                "SELECT * FROM history WHERE language = ?1 ORDER BY timestamp DESC LIMIT ?2",
            )?;
            let mapped = stmt.query_map(params![lang, limit as i64], row_to_entry)?;
            let mut out = Vec::new();
            for row in mapped {
                out.push(row?);
            }
        ...
        None => {
            let mut stmt =
                conn.prepare("SELECT * FROM history ORDER BY timestamp DESC LIMIT ?1")?;
